...
export function hexToBytes(hex) {
    ...
    if (clean.length % 2 !== 0) {
        throw new Error(`hexToBytes: odd-length hex string (length=${clean.length}); hex must have an even number of digits`);
    }
...
export function scoringWeightsToWire(weights) {
    if (!weights)
        return undefined;
    return {
...
export function normalizeServerUrl(url) {
    ...
    try {
        ...
        if (parsed.protocol === "http:" && !isLocal) {
            ...
            console.warn(`[memwal] serverUrl "${trimmed}" uses plaintext HTTP on a non-localhost host. ` +
 ...
 */
export function sanitizeServerError(status, rawBody) {
    if (status === 401) {
        return {
            message: "401 from relayer: typically wrong private key, key not registered on this account, " +
